...
def run_mantis_formatter_pipeline(RUN_NAME, input_param_names, optimized_formulations): 
    n_component_system = len(input_param_names)


    print_slowly(f"\n\n--- FORMATTING {n_component_system}-COMPONENT OPTIMIZED FORMULATIONS FOR MANTIS ---")

    # Define source and destination file paths
    template_dict = {4: os.path.join("exp_templates", "4_comp_template_formulation_sheet.xlsx"),
                     5: os.path.join("exp_templates", "5_comp_LNP_CORRECT_TEMPLATE.xlsx") }

    src = template_dict[n_component_system]
    dst = os.path.join("suggested_LNPs", f"{RUN_NAME}_formulation_sheet.xlsx")
    ...
    # FUNCTIONS TO FOR EACH INDIVIDUAL TEMPLATES.
    def use_4_comp_template():
        ...
        ROW_CONC = {
            "Ionizable_Lipid": 54,  # Ionizable L. Conc.
            "Helper_Lipid":    55,  # Helper L. Conc.
            "Chol":      56,  # Cholesterol Conc.
            "PEG":       57,  # DMG-PEG Conc.
        }
        ROW_VOL = {
            "Ionizable_Lipid": 63,  # SM102 solution (µL)
            "Helper_Lipid":    64,  # Helper lipid solution (µL)
            "Chol":      65,  # Cholesterol solution (µL)
            "PEG":       66,  # DMG-PEG solution (µL)
            "EtOH":   67,  # Needed Ethanol Volume (µL)
        }
        ...
        while ws.cell(row=55, column=col).value is not None:
        ...
        csv_path = f'suggested_LNPs/{RUN_NAME}_mantis_ready.csv'
        # Save to CSV
        df.to_csv(csv_path, index = False)
        print(f"Mantis-ready CSV saved as {csv_path}")
        return

    def use_5_comp_template():
    ...
    #USING TEMPLATE
    #Populate templates
    if n_component_system == 4:
        use_4_comp_template()
    elif n_component_system == 5:
        use_5_comp_template()
    else:
        raise(f"{n_component_system} LNPs ARE NOT VALID/NEW TEMPLATE NEEDED")
