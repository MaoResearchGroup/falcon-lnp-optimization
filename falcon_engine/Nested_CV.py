...
class NESTED_CV:
    ...
    def input_target(self, X, y, data):
        ...
        self.F = self.cell_data['Formula_label'] #used to track LNPs
 ...
 ###### Retrain model using best parameters to evaluate the test set MAE
    def FINAL_TEST_MAE(self):
        ...
        mean_baseline_pred = np.full_like(self.y_final_test, np.mean(self.y_cv_loop))
        median_baseline_pred = np.full_like(self.y_final_test, np.median(self.y_cv_loop))

        mean_baseline_AE = abs(self.y_final_test - mean_baseline_pred)
        median_baseline_AE = abs(self.y_final_test - median_baseline_pred)
        mean_baseline_MAE = float(mean_baseline_AE.mean(axis=0))
        median_baseline_MAE = float(median_baseline_AE.mean(axis=0))
