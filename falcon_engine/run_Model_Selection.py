...
def run_Model_Selection(pipeline):
  ...
  for model_name in model_list:
    ...
    final_AE, final_acc, final_AE_SEM, final_spear, final_pears, pred_df, mean_baseline_MAE, mean_baseline_se, median_baseline_MAE, median_baseline_se = model_instance.FINAL_TEST_MAE() #Train best selected model archetecture and hyperparameters on all training data and evaluate performance on test set
    model_instance.best_model_refit() #Fit best model to all the data as training data 
    ...
    # Save final evaluation metrics in consistent format
    final_metrics_df = pd.DataFrame([{
        'Model_Name': model_name,
        'Model_MAE': final_acc,
        'Model_AE_SEM': final_AE_SEM,
        'Spearman_Rank': final_spear[0],
        'Pearson_R': final_pears[0],
        'Mean_Baseline_MAE': mean_baseline_MAE,
        'Mean_Baseline_AE_SEM': mean_baseline_se,
        'Median_Baseline_AE_MAE': median_baseline_MAE,
        'Median_Baseline_AE_SEM': median_baseline_se
    }])

    with open(save_path + f'{model_name}/Final_Evaluation_Metrics.csv', 'w', encoding='utf-8-sig') as f:
        final_metrics_df.to_csv(f, index=False)
